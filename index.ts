...
async function readFileContent(fileId: string, gid?: string) {
  ...
  if (file.data.mimeType?.startsWith("application/vnd.google-apps")) {
    ...
    const params: any = { fileId, mimeType: exportMimeType };
    if (gid && file.data.mimeType === "application/vnd.google-apps.spreadsheet") {
      params.gid = gid;
    }
  ...
  return {
    tools: [
      ...
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            gid: {
              type: "string",
              description: "Optional: The sheet ID (gid) for Google Sheets",
            },
          },
          required: ["file_id"],
        },
      },
      {
        ...
        description: "List the sheets (tabs) in a Google Spreadsheet",
        inputSchema: {
          type: "object",
          properties: {
            file_id: {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  if (request.params.name === "gdrive_search") {
    ...
    const gid = request.params.arguments?.gid as string;
    if (!fileId) {
      throw new McpError(ErrorCode.InvalidParams, "File ID is required");
    }

    try {
      const result = await readFileContent(fileId, gid);
      return {
        content: [
          {
            type: "text",
  ...
  } else if (request.params.name === "gdrive_list_sheets") {
    const fileId = request.params.arguments?.file_id as string;
    if (!fileId) {
      throw new McpError(ErrorCode.InvalidParams, "File ID is required");
    }
    ...
    try {
      ...
      const sheetList = res.data.sheets
        ?.map((s: any) => `${s.properties.title} (GID: ${s.properties.sheetId})`)
        .join("\n");
      ...
      return {
        content: [
          {
            type: "text",
            text: `Found ${res.data.sheets?.length ?? 0} sheets:\n${sheetList}`,
          },
        ],
        isError: false,
      };
